...
void ATLASspliter(){
	...
	gStyle->SetOptStat(0);
    gStyle->SetErrorX(0);
 	const int binbottom=7;
    const int bintop=66;
    const int nBins = bintop-binbottom;
    const int nAbins=53;
    ...
    for(int i=nBins; i>=0;i--){
    	bins[i] = 2.7-.1*binsetter; 
    	binsetter++;
    }
    double aBins[nAbins+1];
    binsetter=0;
    for (int i = nAbins; i >=0; --i)
    {
    	aBins[i]=2.7 - binsetter*.1;
    	binsetter++;
    }
    const int myBinN = 80;
	...
	while(loadcounter<=66){
		...
		temp2 = star+"="+std::to_string(-2.7+.1*binsetter++);
		htemp = new TH1F(temp.c_str(),temp2.c_str(),nBins,bins);
		htemp->Sumw2();
		for(int bincounter=binbottom;bincounter<=bintop;bincounter++){
			htemp->SetBinContent(bincounter-(binbottom-1),splithists.front()->GetBinContent(bincounter));
			htemp->SetBinError(bincounter-(binbottom-1),splithists.front()->GetBinError(bincounter));
		}
		splithists.pop();
	...
	for(unsigned i=0; i<atlasstar.size();i++){
		...
		temp2 = star+" "+atlasstr;
		htemp = new TH1F(temp.c_str(),temp2.c_str(),nAbins,aBins);
		htemp->Sumw2();
		atlasstar[i] = htemp;
	}
	int clonecounter =0;
	cout<<smallsplit.size()<<'\n';
	...
	{
		//should these be +1?
		atlasstar[0]->SetBinContent(i+1,d90[i]);
		atlasstar[1]->SetBinContent(i+1,d60[i]);
		atlasstar[2]->SetBinContent(i+1,d40[i]);
		atlasstar[3]->SetBinContent(i+1,d30[i]);
		atlasstar[4]->SetBinContent(i+1,d20[i]);
		atlasstar[5]->SetBinContent(i+1,d10[i]);
		atlasstar[6]->SetBinContent(i+1,d5[i]);
		atlasstar[7]->SetBinContent(i+1,d1[i]);
		atlasstar[0]->SetBinError(i+1,e90[i]);
		atlasstar[1]->SetBinError(i+1,e60[i]);
		atlasstar[2]->SetBinError(i+1,e40[i]);
		atlasstar[3]->SetBinError(i+1,e30[i]);
		atlasstar[4]->SetBinError(i+1,e20[i]);
		atlasstar[5]->SetBinError(i+1,e10[i]);
		atlasstar[6]->SetBinError(i+1,e5[i]);
		atlasstar[7]->SetBinError(i+1,e1[i]);
	}
	...
	for(unsigned i=0; i<aClone.size();++i){
		...
		temp = "unmodified"+std::to_string(i);
		tc->SaveAs(temp.c_str());
		aClone[i] = (TH1F*) atlasstar[i]->Clone(temp.c_str());
	}
	/* weight the transformations */
	for(int i=0; i<=nAbins;i++)	
	{
		//should the bins be +1?
		transformations[0][i]->Scale(atlasstar[0]->GetBinContent(i+1));
		transformations[1][i]->Scale(atlasstar[1]->GetBinContent(i+1));
		transformations[2][i]->Scale(atlasstar[2]->GetBinContent(i+1));
		transformations[3][i]->Scale(atlasstar[3]->GetBinContent(i+1));
		transformations[4][i]->Scale(atlasstar[4]->GetBinContent(i+1));
		transformations[5][i]->Scale(atlasstar[5]->GetBinContent(i+1));
		transformations[6][i]->Scale(atlasstar[6]->GetBinContent(i+1));
		transformations[7][i]->Scale(atlasstar[7]->GetBinContent(i+1));
	}
	...
	TFile *finalout =new TFile("calout.root","RECREATE");
	for (int i = 0; i < 8; ++i)
	{
    	calAT[i]->Write("cal-eta");
	}
	finalout->Write();
	finalout->Close();
}