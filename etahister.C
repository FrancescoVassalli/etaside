...
void etahister(){
    ...
    TH1F *h_etanorm = new TH1F("enorm","Eta distribution 5.06 TeV charged final state",nBins,bins);
    ...
    TH1F *h_etaboost = new TH1F("eboost","Eta distribution 5.06",nBins, bins);
    ...
    TH1F *h_fe = new TH1F("hfsys","Eta distribution 5.06 TeV charged final state 60-90 \% with PP ratio",nBins,bins);
    h_f->SetXTitle("Eta");
    ...
    tc->Clear("D");
    h_f->Scale(1/h_f->Integral(),"width");
    h_f->SetMarkerColor(kOrange);
    h_f->SetLineColor(kGreen);
    tl = new TLegend(.4,.7,.6,.9);
    tl->AddEntry(h_etanorm,"pp unboosted","l");
    tl->AddEntry(h_etaboost,"pp boosted","l");
    tl->AddEntry(h_f,"p-Pb uncorrected","l");
    h_etaboost->SetXTitle("Eta");
    h_etaboost->SetYTitle("norm count");
    h_etaboost->Draw("Pe");
    h_etanorm->Draw("Pe same");
    h_f->Draw("Pe same");
    h_fe->Scale(1/h_fe->Integral(),"width");
    h_fe->SetMarkerStyle(3);
    h_fe->SetMarkerColor(kMagenta+2);
    h_fe->SetLineColor(kYellow+3);
    h_fe->Draw("Pe same");
    tl->AddEntry(h_fe,"p-Pb corrected","l");
    tl->Draw();
