...
void splithist(){
	...
	etastar->Scale(1/etastar->Integral());
	dijet_tree->Draw("etaboost>>eta*","","goff");
	etastar->Sumw2();

	TH1F *etaproper = new TH1F("etaproper","#eta distribution 60-90%",myBinN,mybins);
	...
	TH1F *etasolve = new TH1F("solve","calculated #eta distribution",myBinN,mybins);
	...
	etaproper->Draw();
	tc->SaveAs("proper.pdf");
	etastar->Draw();
	tc->SaveAs("etastar.pdf");

	TH1F *residuals = new TH1F("res","calculation residual",myBinN,mybins);
	residuals->Divide(etasolve,etaproper);
	residuals->Draw();
